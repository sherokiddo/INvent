...
def createImageWithName(ItemName, cab):
  img = createNewImage(350,150)
  drawImage = ImageDraw.Draw(img)
  font = ImageFont.truetype("methods\\ArialMT.ttf", 20)
  lines = wrapString(ItemName +'  '+ cab,30)
...
def createImageWithInvNumber(InvNumber):
  ...
  drawImage = ImageDraw.Draw(img)
  font = ImageFont.truetype("methods\\ArialMT.ttf", 18)
...
def itog(name,numb,cab):
  ImageWithName = createImageWithName(name, cab)
  ImageWithNumber = createImageWithInvNumber(numb)
  QrCode = createQrCode(str(cab)+'_'+str(numb))
